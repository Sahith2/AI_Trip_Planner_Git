...
function renderWeather(data, text) {
  ...
  // Try per-day weather column from the itinerary table
  const tables = parseMarkdownTables(text);
  const dayTable = tables.find(t => t[0] && /day/i.test(t[0][0]) && t.length > 1);
  ...
  if (dayTable) {
    ...
    if (weatherCol !== -1) {
      ...
      const cards = rows
        .map((row, i) => {
          const desc = cleanMarkdown(row[weatherCol] || "");
          if (!desc) return "";
          const label = dateCol !== -1 && row[dateCol] ? cleanMarkdown(row[dateCol]) : `Day ${i + 1}`;

          return `
            <div class="weather-card">
              <div class="weather-date">${escapeHtml(label)}</div>
              <div class="weather-main">
                <span class="weather-icon">${weatherIcon(desc)}</span>
              </div>
              <div class="weather-desc">${inlineText(desc)}</div>
            </div>
          `;
        })
        .filter(Boolean)
      ...
      if (cards) {
        container.innerHTML = `<div class="weather-grid">${cards}</div>`;
        return;
      }
    }
  }
