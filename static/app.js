...
function updateTripSummary(data, text) {
  const destination = getSummaryField(text, "destination") || "—";
  const dates = getSummaryField(text, "dates");
  const weather = getSummaryField(text, "weather");
  ...
  if (durationEl) {
    if (dates) {
      ...
      durationEl.textContent = match ? `${match[1]} days` : "—";
    }
  }
